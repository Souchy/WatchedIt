...
pub fn upsert_movie(pg: &mut PgClient, id: i32, v: &MovieDetails) -> Result<(), Box<dyn Error>> {
    pg.batch_execute(
...
pub fn upsert_crew(pg: &mut PgClient, movie_id: i32, crew: &Crew) -> Result<(), Box<dyn Error>> {
    Ok(())
}

pub fn upsert_cast(pg: &mut PgClient, movie_id: i32, cast: &Cast) -> Result<(), Box<dyn Error>> {
    pg.batch_execute(
        "
